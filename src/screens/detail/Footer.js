...
class Footer extends Component {
  ...
  render() {
    ...
    const { animatedBackgroundColor, animatedPriceText, animatedPriceInfoText } = this._getAnimated();

    return (
      <AnimatedFooter style={ [styles.footer, { backgroundColor: animatedBackgroundColor }] }>
        <View>
          <Animated.Text style={ [styles.footerPriceText, { color: animatedPriceText }] }>$30</Animated.Text>
          <Animated.Text style={ [styles.footerPriceInfo, { color: animatedPriceInfoText }] }>Per Guest</Animated.Text>
        </View>
        <ArrowRoundedButton
